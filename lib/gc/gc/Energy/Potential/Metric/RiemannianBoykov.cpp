    ...
    namespace Energy
    {
        namespace Potential
        {
            namespace Metric
            {
                ...
                /** @cond */
                template <Size N, class T> static inline T RiemannCoef(const T &val);

                template <> inline Float32 RiemannCoef<2,Float32> (const Float32 &val)
                {
                    return Math::Pow(val, 1.5f);
                }

                template <> inline Float64 RiemannCoef<2,Float64> (const Float64 &val)
                {
                    return Math::Pow(val, 1.5);
                }

                template <> inline Float32 RiemannCoef<3,Float32> (const Float32 &val)
                {
                    return val * val;
                }

                template <> inline Float64 RiemannCoef<3,Float64> (const Float64 &val)
                {
                    return val * val;
                }
                /** @endcond */
                ...
                template <Size N, class T>
                RiemannianBoykov<N,T> &RiemannianBoykov<N,T>::SetTransformationMatrix
                ...
                template <Size N, class T>
                RiemannianBoykov<N,T> &RiemannianBoykov<N,T>::SetMetricTensor
                    (const Math::Algebra::SquareMatrix<N,T> &mt)
                {
