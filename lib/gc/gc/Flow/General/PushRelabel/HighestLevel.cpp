	...
	namespace Flow
	{
        namespace General
        {
            namespace PushRelabel
            {
		        ...
		        void HighestLevel<TFLOW,TCAP>::Init(Size nodes, Size max_arcs, Size src_arcs, Size snk_arcs)
		        {
			        Dispose();
		        ...
		        template <class TFLOW, class TCAP>
		        void HighestLevel<TFLOW,TCAP>::SetArcCap(Size n1, Size n2, TCAP cap, TCAP rcap)
		        {
                    if (m_stage != 1)
                    {
		        ...
		        template <class TFLOW, class TCAP>
		        void HighestLevel<TFLOW,TCAP>::SetTerminalArcCap(Size node, TFLOW csrc, TFLOW csnk)
                {
                    if (m_stage != 1)
                    {
                        throw System::InvalidOperationException(__FUNCTION__, __LINE__,
		        ...
		        template <class TFLOW, class TCAP>
		        TFLOW HighestLevel<TFLOW,TCAP>::FindMaxFlow()
		        {
                    if (m_stage != 1)
                    {
                        throw System::InvalidOperationException(__FUNCTION__, __LINE__,
		        ...
		        template <class TFLOW, class TCAP>
                void HighestLevel<TFLOW,TCAP>::GapRelabeling(Layer *empty_lay, Size next_layer_rank)
                {
                    Node *node;
                    Layer *lay;
		        ...
		        template <class TFLOW, class TCAP>
		        void HighestLevel<TFLOW,TCAP>::GlobalUpdate()
		        {
                    Node *node, *head;
                    Layer *lay;
                    Arc *arc;
                    ...
                    for ( ; lay->m_first_active != NULL || lay->m_first_inactive != NULL; lay++)
                    {
                        ...
                        for (node = lay->m_first_active; node != NULL; node = node->m_lay_next)
                        {
                            for (arc = node->m_first; arc != NULL; arc = arc->m_next)
                            {
                                if (arc->m_sister->m_res_cap > 0)
                                {
                                    head = arc->m_head;
                                    if (head->m_rank == m_nodes)
                                    {
		        ...
		        template <class TFLOW, class TCAP>
		        Origin HighestLevel<TFLOW,TCAP>::NodeOrigin(Size node) const
		        {
                    if (m_stage != 2)
                    {
                        throw System::InvalidOperationException(__FUNCTION__, __LINE__,
		        ...
		        template <class TFLOW, class TCAP>
		        void HighestLevel<TFLOW,TCAP>::Dispose()
		        {
			        m_node_list.Dispose();
                ...
                /** @cond */
		        template class GC_DLL_EXPORT HighestLevel<Int32,Int32>;
		        template class GC_DLL_EXPORT HighestLevel<Float32,Float32>;
		        template class GC_DLL_EXPORT HighestLevel<Float64,Float64>;
                /** @endcond */
            }
        }
	}
